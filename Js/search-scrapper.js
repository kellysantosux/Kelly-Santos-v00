...
async function performSearch(passedQuery) {
    ...
    const searchTerm = (passedQuery || (input ? input.value : '')).trim();
    const resultsContainer = document.getElementById('search-results');
    const searchHeader = document.getElementById('search-term-header');
    
    // Elements to show/hide
    const detailsWrapper = document.getElementById('results-details-wrapper');
    const charWarning = document.querySelector('.search-results-characters');

    if (searchHeader) searchHeader.innerText = searchTerm;
    
    // IF SEARCH TERM IS LESS THAN 3 CHARACTERS
    if (searchTerm.length < 3) {
        // Clear previous list item results cleanly
        if (resultsContainer) resultsContainer.innerHTML = '';
        
        // Hide the details structure
        if (detailsWrapper) detailsWrapper.style.display = 'none';
        
        // Show the character warning block beneath the form input
        if (charWarning) charWarning.style.display = 'block';
        return;
    }

    // VALID ENTRY (3+ Characters): Hide warning block, show search structure
...
function displayResults(matches, searchTerm) {
    ...
    // Update results quantity metadata layout block
    const qtyContainer = document.getElementById('search-term-qty');
    if (qtyContainer) {
        qtyContainer.textContent = matches.length;
    }
        ...
        li.innerHTML = `
            <a class="search-results-card" href="${match.url}">
                <div class="search-breadcrumb">
                    ${match.name}${match.heading ? '<span class="sep"> / </span>' : ''}
